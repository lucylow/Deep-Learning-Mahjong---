...
import { streamStrategyChat } from "../mahjong-chat";
...
async function startServer() {
  ...
  app.post("/api/mahjong/strategy-chat/stream", async (req, res) => {
    ...
    res.flushHeaders();
    try {
      await streamStrategyChat(req.body, controller.signal, (delta) => { if (!finished && !res.writableEnded) res.write(`data: ${JSON.stringify({ delta })}\\n\\n`); });
      if (!finished && !res.writableEnded) { finished = true; res.write("data: [DONE]\\n\\n"); res.end(); }
    } catch (error) {
